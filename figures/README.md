...
Regenerate any figure from the manuscript's own tabulated values:
    ...
    python figures/make_fig4_evidence.py       # Fig4_evidence.png
    python elr/examples/worked_example.py      # writes results.json
    python elr/examples/make_figure.py         # Fig3_evaluation.png
...
**One colour, one meaning, across every panel of a figure.** Fig3 previously
...
is why the severity bars in Fig3(b) and the zero rows in Fig4(b) are a single
...
six rows of Table 3 that state a numeric dataset size; panel (b) is computed
(Wilson score interval width). Fig4 reproduces the subgroup sensitivities in
Section 8 and the column totals of Table 2. Fig3 is generated from
`results.json`, written by the evaluation harness, so the figure and the run
cannot disagree.

Fig1 (workflow) and Fig5 (roadmap) are diagrams, not plots: their text is
